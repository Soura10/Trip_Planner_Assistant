...
# default origin airport IATA code KOLKATA (CCU)
...
def clean_text(text:str) -> str:
    """
    clean the text by remonving special characters ,  extra spaces and new lines
    
    """
    # remove special character and extra spaces
    ...
    # go through every word s in text , and keep it only if w is NOT in stop_words_list
    return " ".join(words).strip() # " ".join(words) puts the words back into a sentence, with space between each word, and strip() removes extra spaces from beginning and end of the sentence.


def country_name_to_code(text:str) -> str:
    # purpose : convert a country name to an ISO letter country code
    text  = clean_text(text)

    #check your own country aliases first
    if text in COUNTRY_ALIASES:
        return COUNTRY_ALIASES[text]
    
    #try pycountry lookup
    try:
        country = pycountry.countries.lookuup(text)
        return country.alpha_2
    except LookupError:
        pass  # if not found, return None

    # look for a country name inside longer text
...
def airport_country_matches(airport:dict, country_code: str) -> bool:
    # determine whether a particular airport belongs to a particular country, based on the airport's country code and the provided country code.

    # basically, get the airport's country , make sure it's a string, removes extra spaces, and convert it to uppercase
    airport_country = str(airport.get("country" " ")).upper().strip()

    # now it compares the airport's country with the requested country code
    if airport_country == country_code:
        return True

    #  if it is not matched, give the country code whose 2-letter code is country code

    try:
         country = pycountry.countries.get(alpha_2 = country_code)
         if country and airport_country == country.name.upper():
             return True
    except Exception:
        pass
...
if __name__ == "__main__":
    print(search_flights("plan a trip from kolkata to bangalore for 7 days"))
    print("\n" + "=" * 80 + "\n")
